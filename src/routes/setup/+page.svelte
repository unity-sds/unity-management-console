<script lang="ts">
  ...
  const venueAndProjectStore = derived([venueStore, projectStore], ([$venueStore, $projectStore]) => {
  ...
  const transformedPrivateOptions = privateOptions.map(option => ({ value: option, label: option }));
  const transformedPublicOptions = publicOptions.map(option => ({ value: option, label: option }));
    ...
    <div class="w-full lg:w-3/4 px-4">
      {#if $installRunning === false}
        <form class="space-y-4">
          ...
          <SelectField label="Public Subnets" id="publicSubnets" multiple={true} options={transformedPublicOptions}
